import {
    javascript,
    typescript,
    html,
    css,
    reactjs,
    redux,
    tailwind,
    nodejs,
    mongodb,
    git,
    figma,
    docker,
    carrental,
    exercise,
    gpt,
    threejs,
  } from "../assets";
  
  export const navLinks = [
    {
      id: "about",
      title: "About",
    },
    {
      id: "work",
      title: "Work",
    },
    {
      id: "contact",
      title: "Contact",
    },
  ];
  
  const technologies = [
    {
      name: "HTML 5",
      icon: html,
    },
    {
      name: "CSS 3",
      icon: css,
    },
    {
      name: "JavaScript",
      icon: javascript,
    },
    {
      name: "TypeScript",
      icon: typescript,
    },
    {
      name: "React JS",
      icon: reactjs,
    },
    {
      name: "Redux Toolkit",
      icon: redux,
    },
    {
      name: "Tailwind CSS",
      icon: tailwind,
    },
    {
      name: "Node JS",
      icon: nodejs,
    },
    {
      name: "MongoDB",
      icon: mongodb,
    },
    {
      name: "Three JS",
      icon: threejs,
    },
    {
      name: "git",
      icon: git,
    },
    {
      name: "figma",
      icon: figma,
    },
    {
      name: "docker",
      icon: docker,
    },
  ];

  
  const projects = [
    {
      name: "CarHub",
      description:
        "A user-friendly web application for searching specific vehicle models and accessing detailed specifications.",
      tags: [
        {
          name: "nextjs",
          color: "blue-text-gradient",
        },
        {
          name: "typescript",
          color: "green-text-gradient",
        },
        {
          name: "tailwind",
          color: "pink-text-gradient",
        },
      ],
      image: carrental,
      source_code_link: "https://github.com/SpoonMouse/rent-mobile",
      live_link: "https://carhubdomtracy.netlify.app"
    },
    {
      name: "ExerciseBase",
      description:
        "A web app that allows athletes to search and filter exercises by muscle and equipment for personalized workouts.",
      tags: [
        {
          name: "react",
          color: "blue-text-gradient",
        },
        {
          name: "restapi",
          color: "green-text-gradient",
        },
        {
          name: "materialui",
          color: "pink-text-gradient",
        },
      ],
      image: exercise,
      source_code_link: "https://github.com/SpoonMouse/exercise-app",
      live_link: "https://exercisedomtracy.netlify.app"
    },
    {
      name: "GPT-3 Landing Page",
      description:
        "Stunning and informative platform showcasing the power of GPT-3, adapted from a captivating Figma design",
      tags: [
        {
          name: "figma",
          color: "blue-text-gradient",
        },
        {
          name: "react",
          color: "green-text-gradient",
        },
        {
          name: "css",
          color: "pink-text-gradient",
        },
      ],
      image: gpt,
      source_code_link: "https://github.com/SpoonMouse/chatgpt3-website",
      live_link: "https://gpt3domtracy.netlify.app"
    },
  ];
  
  export { technologies, projects };